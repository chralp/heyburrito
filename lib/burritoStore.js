module.exports = ((redis,client)=> {
    ...
    const lastResetKey = username => `${username}:lastReset`


    const getRecieved = user => getter(receivedKey(user))
    const getGiven = user => getter(givenKey(user))
    const getLastReset = user => getter(lastResetKey(user))
    ...
    const getUserScores = () => {
        return new Promise((rs, rj) => {
            client.keys("*:received", (err, keys) => {
                ...
                if (keys) {
                    const waitFor = keys.map(key => {
                        return getter(key).then(score => ({
                            user: key.split(":")[0],
                            score
                        }));
                    });
